...
FindFreeSpace:
    TXA                     ;
    CMP.l DZ.DSFirstSlot    ;
    BNE +                   ;if(X == first)
RTS                         ;   return
+
    
    PHX                     ;
    LDA.l DZ.DSFirstSlot    ;i = first
    TAX                     ;
-
...
++                              ;{
    PLX                         ;
                                ;
    LDA.l DZ.DSLocPreviewSlot,x ;   if(i == first)
    BPL +                       ;   {
                                ;
    LDA.l DZ.DSFindSpaceMethod  ;       if(FindSpaceMethod == Top To Bottom)
    BNE ++                      ;
    LDA #$00                    ;           InitialOffset = 0;
    BRA +++                     ;       
++                              ;       else
    LDA.l DZ.DSMaxSpace         ;           IntitialOffset = Max Space;
    BRA +++                     ;   }
+                               ;   else
    PHX                         ;   {
    TAX                         ;
                                ;       if(FindSpaceMethod == Top To Bottom)
    LDA.l DZ.DSFindSpaceMethod  ;       
    BNE ++                      ;
                                ;
    LDA.l DZ.DSLocSpaceUsedOffset,x ;       InitialOffset = i.preview.OffSet + i.preview.SpaceUsed;
    CLC                             ;
    ADC.l DZ.DSLocSpaceUsed,x       ;
    PLX                             ;
    BRA +++                         ;   else
++                                  ;
    LDA.l DZ.DSLocSpaceUsedOffset,x ;       InitialOffset = i.preview.OffSet;
    PLX                         ;   }
+++                             ;
    PHA                         ;
    ...
    BNE ++                          ;{
                                    ;
    LDA.l DZ.DSLocSpaceUsedOffset,x ;   if(i.OffSet - InitialOffset < RequiredSpace)
    SEC                             ;   
    SBC $01,s                       ;       Go To Next
    CMP !ScratchF                   ;
    BCC .next                       ;   else
...
++                                  ;
    PLA                             ;else
    SEC                             ;{
    SBC.l DZ.DSLocSpaceUsedOffset,x ;   if(InitialOffset - i.OffSet - i.SpaceUsed < RequiredSpace)       
    SEC
    SBC.l DZ.DSLocSpaceUsed,x       ;
    ...
    LDA $01,s                       ;       
    TAX                             ;   
    LDA.l DZ.DSLocPreviewSlot,x     ;       MoveAt(X, i.preview);
    JSR MoveAt                      ;
    PLX                             ;
    PLX                             ;       return;
RTS                                 ;   }
...
GetFirstSpace:
    ...
    BNE +
    LDA #$00
    STA.l DZ.DSLocSpaceUsedOffset,x     ;   X.Offset = 0;
RTS
+
    LDA.l DZ.DSMaxSpace                 ;else
    SEC
    SBC.l DZ.DSLocSpaceUsed,x
    STA.l DZ.DSLocSpaceUsedOffset,x     ;   X.Offset = MaxSpace - X.SpaceUsed;
RTS
;X = Slot
AssignSpaceBasedOnSlot2:
    LDA.l DZ.DSLocPreviewSlot,x         ;if(X == first)
    BPL +                               ;{
                                        ;
...
.Start
    ...
    TAX                                 ;i = X.preview;
-
    LDA.l DZ.DSLocUsedBy,x              ;if(i.UsedBy == null || !i.isValid())
    CMP #$FF                            ;{
    BEQ +                               ;
                                        ;
    PHX                                 ;
...
++                                      ;{
    ...
    BNE +                               ;
                                        ;       
    LDA.l DZ.DSLocSpaceUsedOffset,x     ;       
    CLC                                 ;
    ADC.l DZ.DSLocSpaceUsed,x           ;
    PLX                                 ;
    STA.l DZ.DSLocSpaceUsedOffset,x     ;       X.Offset = i.Offset + i.SpaceUsed;
                                        ;
    BRA ++                              ;
+                                       ;   else
    LDA.l DZ.DSLocSpaceUsedOffset,x     ;
    ...
    PLX                                 ;
    SBC.l DZ.DSLocSpaceUsed,x           ;
    STA.l DZ.DSLocSpaceUsedOffset,x     ;}
++                                      ;
                                        ;
    LDA.l DZ.DSLocSpaceUsedOffset,x     ;
...
FindSlot:
    ...
    LDX #$2F                    ;\X = 2F
-                               ;|
    LDA.l DZ.DSLocUsedBy,x      ;|check if slot is used or clear
    CMP #$FF                    ;|
    BEQ +                       ;|
    BIT #$80
    BNE ++
...
+
    ...
    TXA                         ;if(Length == 1)
    STA.l DZ.DSFirstSlot        ;first = last = X;
    STA.l DZ.DSLastSlot         ;
    LDA #$FF                    ;
    STA.l DZ.DSLocNextSlot,x    ;X.next = X.preview = null;
    STA.l DZ.DSLocPreviewSlot,x ;
RTS
+
    PHX                         ;else                   //Preserve X
    LDA.l DZ.DSFirstSlot        ;                       
    STA.l DZ.DSLocNextSlot,x    ;X.next = first;

    TAX                         ;                       //X reg = First Slot
    PLA                         ;first.preview = X;     //A = X
    STA.l DZ.DSLocPreviewSlot,x ;                       //first.Preview = X
    TAX                         ;                       //X reg = X

    LDA #$FF                    ;
    STA.l DZ.DSLocPreviewSlot,x ;X.preview = null;      

    TXA                         ;                       //A = X
    STA.l DZ.DSFirstSlot        ;first = X;             //first = X
...
+
    PHX                         ;else
    LDA.l DZ.DSLastSlot         ;
    STA.l DZ.DSLocPreviewSlot,x ;X.preview = last;

    TAX                         ;
    PLA                         ;last.next = X;
    STA.l DZ.DSLocNextSlot,x    ;
    TAX                         ;

    LDA #$FF                    ;
    STA.l DZ.DSLocNextSlot,x    ;X.next = null;

    TXA                         ;
    STA.l DZ.DSLastSlot         ;last = X;
...
+                                   ;
    ...
    STA.l DZ.DSLocNextSlot,x    ;X.next = X.preview = null;
    STA.l DZ.DSLocPreviewSlot,x ;
    STA.l DZ.DSFirstSlot        ;first = last = null;
    STA.l DZ.DSLastSlot         ;
    STA.l DZ.DSLocUsedBy,x      ;X.UsedBy = FF;
RTS
+
    LDA.l DZ.DSLocPreviewSlot,x
    BMI +
    ...
    LDA.l DZ.DSLocNextSlot,x        ;                               //A = X.next                     
    PLX                             ;                               //X reg = X.preview
    STA.l DZ.DSLocNextSlot,x        ;                               //X.preview.next = X.next;
    PLX                             ;                               //X reg = X
    BRA ++
+
    LDA.l DZ.DSLocNextSlot,x        ;else
    STA.l DZ.DSFirstSlot            ;   first = X.next;
++
    ...
    LDA.l DZ.DSLocPreviewSlot,x     ;                                   //A = X.preview                     
    PLX                             ;                                   //X reg = X.preview
    STA.l DZ.DSLocPreviewSlot,x     ;                                   //X.next.preview = X.preview;
    PLX                             ;                                   //X reg = X
    BRA ++
+
    LDA.l DZ.DSLocPreviewSlot,x     ;else
    STA.l DZ.DSLastSlot             ;   last = X.preview;
++
...
MoveAt:
    ...
    PLA                     ;
    STA.l DZ.DSLocUsedBy,x  ;X.UsedBy = aux;
...
+                               ;
    ...
    LDA.l DZ.DSLocNextSlot,x    ;                                   //A = position -1.next
    PLX                         ;                                   //X reg = X
    STA.l DZ.DSLocNextSlot,x    ;   X.next = position - 1.next;     //X.next = position - 1.next
                                ;
    CMP #$80                    ;
    BCC +                       ;   if(X.next == null)
                                ;
    TXA                         ;                                   //A = X
    STA.l DZ.DSLastSlot         ;       last = X;                   /last = X
    BRA ++                      ;
+                               ;   else
    PHA                         ;                                   //A = X.next, preserve A
    TXA                         ;                                   //A = X
    PLX                         ;                                   //X reg = X.next
    STA.l DZ.DSLocPreviewSlot,x ;       X.next.preview = X;         //x.next.preview = X;
    TAX                         ;                                   //X reg = X
++                              ;
                                ;
    LDA $01,s                   ;                                   //A = position - 1
    STA.l DZ.DSLocPreviewSlot,x ;   X.preview = position - 1        //X.preview = position - 1 

    TXA                         ;                                   //A = X
    PLX                         ;                                   //X reg = position - 1
    STA.l DZ.DSLocNextSlot,x    ;   position - 1.next = X           //position - 1.next = X
    TAX                         ;                                   //X reg = X
...
ClearSlot:
    LDA #$00
    STA.l DZ.DSTotalDataSentOdd
    STA.l DZ.DSTotalDataSentEven
    STA.l DZ.DSTotalSpaceUsedOdd
    STA.l DZ.DSTotalSpaceUsedEven
    STA.l DZ.DSTotalSpaceUsed
...
+                               ;/
    ...
    PHX                     ;\
    LDA.l DZ.DSFirstSlot    ;| x = last slot
    TAX                     ;|
-                           ;/
    LDA.l DZ.DSLocUsedBy,x      ;\
...
.ret
    ...
    LDA.l DZ.DSTotalSpaceUsed       ;\
    CLC                         ;|
    ADC.l DZ.DSTotalSpaceUsedOdd    ;|
    CLC
    ADC.l DZ.DSTotalSpaceUsedEven   ;|
    STA.l DZ.DSTotalSpaceUsed       ;/Total Space Used = Space used by 60fps sprites + space used by 30fps sprites
...
.IsUsed
    ...
    LDA.l DZ.DSLocFrameRateMethod,x    ;\
    BEQ +                       ;|Check if is any frame (60fps) or not (30fps)
    DEC A                       ;/

    PHX                         ;\
    TAX                         ;|X = 0 (only odd frames) or 1 (only even frames)
    LDA.l DZ.DSTotalSpaceUsedOdd,x  ;| 
    CLC                         ;|
    ADC $02,s                   ;|
    STA.l DZ.DSTotalSpaceUsedOdd,x  ;|DZ.DSTotalSpaceUsedOdd (or DSTotalSpaceUsedEven) += Space used                          ;/

    LDA $02,s                   ;\
    CLC                         ;|
    ADC.l DZ.DSTotalDataSentOdd,x   ;|Total data += space used
    STA.l DZ.DSTotalDataSentOdd,x   ;/
    PLX
    PLA
...
+
    LDA $01,s                   ;\
    CLC                         ;|
    ADC.l DZ.DSTotalSpaceUsed       ;|
    STA.l DZ.DSTotalSpaceUsed       ;/DZ.DSTotalSpaceUsed += space used

    LDA $01,s                   ;\
    CLC                         ;|
    ADC.l DZ.DSTotalDataSentOdd     ;|Total data += space used
    STA.l DZ.DSTotalDataSentOdd     ;/

    PLA                         ;\
    CLC                         ;|
    ADC.l DZ.DSTotalDataSentEven    ;|Total data += space used
    STA.l DZ.DSTotalDataSentEven    ;/
RTS
...
CheckSlot:
    ...
    CLC                         ;|
    ADC.l DZ.DSTotalSpaceUsed   ;|
    CMP.l DZ.DSMaxSpace         ;|if Total Space + space used for this sprite > max space then ret
    BEQ +                       ;|
    BCC +                       ;|
                                ;|
    CLC                         ;|
...
+
    ...
    LDA.l DZ.DSTotalSpaceUsedOdd    ;if off is less than even then x = 0
    CMP.l DZ.DSTotalSpaceUsedEven
    BCS +++
    DEX  
+++
    BRA ++                      ;Jump to check
...
++
    ...
    LDA !csNumberOf16x16Tiles
    CLC
    ADC.l DZ.DSTotalDataSentOdd,x
    CMP.l DZ.MaxDataPerFrameIn16x16Tiles      ;check if is safe increase data
    BCC ++
    BEQ ++
...
++
    ...
    LDA !csNumberOf16x16Tiles
    CLC
    ADC.l DZ.DSTotalSpaceUsedOdd,x
    STA.l DZ.DSTotalSpaceUsedOdd,x  ;update space used and data send for 30fps sprite
...
RTL
+
    STA.l DZ.DSTotalDataSentEven    ;update space used and data send for 60fps sprite
...
++
    TXA
    LDX !Scratch0
    STA.l DZ.DSLocFrameRateMethod,x    ;Set Frame rate Method (any, odd or even)

    LDA !csSpriteNumber
    STA.l DZ.DSLocSpriteNumber,x       ;Set sprite number

    LDA !csSpriteTypeAndSlot
    STA.l DZ.DSLocUsedBy,x             ;Set type and slot
    AND #$1F
    STA !Scratch45
    LDA !csSpriteTypeAndSlot
    CLC                         ;|
    ...
    LDA #$FF
    STA.l DZ.DSLocNextSlot,x     ;set as last slot

    LDA #$00
    STA.l DZ.DSLocIsValid,x            ;Sprite is Invalid, must wait dynamic routine
    STA.l DZ.DSLocSharedUpdated,x      ;If the sprite is Shared Dynamic, it is not updated yet

    LDA !csNumberOf16x16Tiles
    STA.l DZ.DSLocSpaceUsed,x          ;Set number of 16x16 tiles
...
FindSpace:
    ...
    LDA.l DZ.DSLocUsedBy,x             ;Set type and slot
    AND #$1F
    STA !Scratch45
    LDA.l DZ.DSLocUsedBy,x
    CLC                         ;|
...
+
    STZ !ScratchB
    LDA $01,s
    CMP.l DZ.DSLocSpaceUsedOffset,x
    BEQ +
    LDA #$01
    STA !ScratchB
+
...
macro Transfer(offset, size, srcOffset)
    ...
    LDA #$00
    STA.l DZ.PPUMirrors.VRAMTransferSourceBNK+$01,x
...
.OneTransfer
    ...
    INC A                       ;|
    STA.l DZ.PPUMirrors.VRAMTransferLength   ;|Number of transfer ++
    ASL                         ;|
    TAX                         ;/X = number of transfer*2

    LDA !drBNK                  ;\
    STA.l DZ.PPUMirrors.VRAMTransferSourceBNK,x            ;|BNK (low byte) = source bnk
    ...
    LDA Sizes,y                 ;\
    CLC                         ;|
    ADC.l DZ.DSStartingVRAMOffset   ;|MapVRAMOffset = Starting VRAM Offset + VRAM Offset
    STA.l DZ.PPUMirrors.VRAMTransferOffset,x     ;/
    ...
    LDA !drAddr                 ;\
    STA.l DZ.PPUMirrors.VRAMTransferSource,x       ;/MapAddr = Addr
    SEP #$20
...
+
    ...
    LDA !drBNK                  ;\
    STA.l DZ.PPUMirrors.VRAMTransferSourceBNK,x            ;|
    STA.l DZ.PPUMirrors.VRAMTransferSourceBNK+$02,x        ;|Set bank for both transfers
    LDA #$00                    ;|
    STA.l DZ.PPUMirrors.VRAMTransferSourceBNK+$01,x        ;|
    STA.l DZ.PPUMirrors.VRAMTransferSourceBNK+$03,x        ;/
    ...
    LDA !drAddr                 ;\
    STA.l DZ.PPUMirrors.VRAMTransferSource,x       ;|MAPResourceOffset of first transfer = Addr of first transfer
    CLC                         ;|
    ADC Sizes,y                 ;|
    STA.l DZ.PPUMirrors.VRAMTransferSource+$02,x   ;/MAPResourceOffset of second transfer = Addr of second transfer
    
    LDY $0003|!dp               ;\
    LDA Sizes,y                 ;|MAPLength = Size of second transfer
    STA.l DZ.PPUMirrors.VRAMTransferSourceLength+$02,x     ;/

    LDY $0002|!dp
    LDA Sizes,y                 ;\
    CLC                         ;|;|MAPVRAMOffset of second transfer = Starting VRAM Offset + Vram Offset of second transfer
    ADC.l DZ.DSStartingVRAMOffset   ;|
    STA.l DZ.PPUMirrors.VRAMTransferOffset+$02,x ;/
...
GetVramDisp:
    PHX
    TAX
    LDA DZ.DSLocSpaceUsedOffset,x
    TAX
    LDA.l VramDisp,x
    CLC
    ADC DZ.DSStartingVRAMOffset8x8Tiles
    PLX
RTL
...
elseif <type> == $20
    STA.l DZ.DSLocUSCluster,x
elseif <type> == $40
    STA.l DZ.DSLocUSExtended,x
else
    STA.l DZ.DSLocUSOW,x
endif
    PLA
    PLA
    SEC
...
elseif <type> == $40
    LDA.l DZ.DSLocUSExtended,x
else
    LDA.l DZ.DSLocUSOW,x
endif
    TAX
    LDA.l DZ.DSLocUsedBy,x
    PLX
